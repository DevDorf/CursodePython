...
def eh_perfeito(n):
	if n >= 2:
		div = [numero for numero in range(1, n) if n % numero == 0]
	else:
		return False
	return sum(div) == n
...
def lista_perfeitos(n):
	if n >= 2:
		return sorted([num for num in range(2, n) if eh_perfeito(num)])
	else:
		return []
